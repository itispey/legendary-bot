...
local function collect_stats(msg)
	...
	if msg.chat.type ~= 'private' and msg.chat.type ~= 'inline' and msg.from then
		...
		db:hincrby('chat:'..msg.chat.id..':usermsgs', msg.from.id, 1)
...
سلام به همگی دوستان! من ربات لجندری هستم :)
به نظر میرسه گروه شما عضو کمی داره... برای اینکه من بتونم توی گروه شما کار کنم، گروه رو به سوپرگروه آپدیت کنید!
اگر تلگرام شما به آخرین نسخه بروز می باشد، کافیست گروه را از حالت خصوصی به عمومی (Public) تغییر دهید... سپس مجدد ربات را اضافه کنید.
وقتی ربات فعال شد، می توانید دوباره گروه خود را به حالت خصوصی تنظیم کنید.
]]))
...
local function parseMessageFunction(update)
	...
	if update.message or update.edited_message then
		...
		elseif msg.sticker then
			msg.media = true
			if msg.sticker.is_animated then
				msg.media_type = 'sticker_animated'
			else
